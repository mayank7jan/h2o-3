  ...
  private class GAMDriver extends Driver {
    ...
    Frame adaptTrain(GAMModel model) {
      ...
      for (int frameInd = 0; frameInd < numGamFrame; frameInd++) {  // append the augmented columns to _train
        ...
        Scope.track(gamFramesKey[frameInd].get());
      }
      return _train;
    }
    ...
    public final void buildModel() {
     ...
     Frame newTFrame = new Frame(adaptTrain(model));  // get frames with correct predictors and spline functions
      DKV.put(newTFrame);
      if (_parms._saveGamCols)
        model._output._gamTransformedTrain = newTFrame._key;
      else
